 ...
 export default function Navbar() { const items = useCartStore((state) => state.items);
   const supabase = createClient();
    const [businessName, setBusinessName] = useState("Sweet Bakery");
     const totalItems = items.reduce( (sum, item) => sum + item.quantity, 0 ); useEffect(() => { async function fetchSettings() { const { data } = await supabase .from("settings") .select("business_name") .limit(1) .single(); if (data?.business_name) { setBusinessName(data.business_name); } } fetchSettings(); }, [supabase]);
return (
